...
;;
;; NOTE (Emacs 29 compat + no-grammar-management):
;; - This version is intended to run on Emacs 29+.
;; - It DOES NOT add anything to `treesit-language-source-alist'.
;; - It DOES NOT call `treesit-ensure-installed'.
;; - If the Lua grammar is not available, it signals a clear error and
;;   suggests using `lua-mode'.

;;; Code:

(require 'comint)

;; Emacs 29+ 才有 treesit；如果没有就直接报错
(unless (require 'treesit nil t)
  (error "lua-ts-mode requires Emacs with tree-sitter support (Emacs 29+)"))

;; c-ts-common 在 Emacs 29 通常存在；但做软依赖以增强兼容性
(require 'c-ts-common nil t)

;; Emacs 30 有这个宏；Emacs 29 没有就跳过
...
(defun lua-ts--treesit-ready-p ()
  "Return non-nil if Lua tree-sitter is available and ready.
This function does NOT install grammars."
  (cond
...
(defvar lua-ts--simple-indent-rules
  `((lua
	 ...
	 ;; Emacs 30 有 c-ts-common-baseline-indent-rule（且是 list 規則）
     ;; Emacs 29 可能沒有 / 或不是 list，這裡只在「確定是 list」時才插入
     ,@(when (and (boundp 'c-ts-common-baseline-indent-rule)
                  (listp c-ts-common-baseline-indent-rule))
         (list c-ts-common-baseline-indent-rule)))))
...
\\{lua-ts-mode-map}"
  ...
  (if (lua-ts--treesit-ready-p)
      (progn
        ...
        ;; Indent.
        (when (boundp 'c-ts-common-indent-offset)
          (setq-local c-ts-common-indent-offset 'lua-ts-indent-offset))
        (when (boundp 'c-ts-common-list-indent-style)
    ...
    (user-error "Lua tree-sitter grammar is not available. Install the Lua grammar first, or use lua-mode"))

  (add-hook 'flymake-diagnostic-functions #'lua-ts-flymake-luacheck nil 'local))

;; Emacs 29/30 兼容：如果没有这个函数就不加 parents
